...
export default function HelpRequired() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await api.upload("/help", fd);

      if (data.success) {
        setToast({ type: "success", message: "Your help request has been submitted!" });
        setFormData({ full_name: "", dob: "", gender: "", phone: "", email: "", emergency: "", help_types: [], zip_code: "", address: "", state: "", district: "" });
      ...
      <form className="max-w-3xl w-full bg-white/70 backdrop-blur-xl p-8 rounded-2xl shadow-lg border border-white/40 space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} placeholder="Enter your full name" className={inp("full_name")} />
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Enter your email address" className={inp("email")} />
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" name="zip_code" value={formData.zip_code} onChange={handleChange} placeholder="Enter zip code" className={inp("zip_code")} />
          </div>
          <div>
            <label className="block mb-1 font-medium text-gray-700">Address</label>
            <div className="relative">
              <FaHome className="absolute left-3 top-3 text-gray-400" />
              <input type="text" name="address" value={formData.address} onChange={handleChange} placeholder="Enter your address" className={`${inp("address")} pl-10`} />
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" name="state" value={formData.state} onChange={handleChange} placeholder="Enter state" className={inp("state")} />
          </div>
          <div>
            <label className="block mb-1 font-medium text-gray-700">District</label>
            <input type="text" name="district" value={formData.district} onChange={handleChange} placeholder="Enter district" className={inp("district")} />
          </div>
        </div>
